...
function extractGameState(): GameState | null {
    try {
        ...
        const mapTileClassToColor = (tileElement: Element): string => {
            ...
            if (tileTypeClass) {
                switch (tileTypeClass) {
                    case 'tile0': return 'firstplayer'; 
                    case 'tile1': return 'black';
                    case 'tile2': return 'white';    // Maps to turquoise icon in popup via 'white' key
                    case 'tile3': return 'blue';
                    case 'tile4': return 'yellow';
                    case 'tile5': return 'red';
                    default:
                        console.warn(`Unknown tile class for color mapping: ${tileTypeClass} on element ${tileElement.id}`);
                        return ''; 
                }
            }
            return ''; 
        ...
        if (centerElement && centerElement.classList.contains('factory-center')) {
            ...
            console.warn('Center element (#factory0 with class .factory-center) not found or missing class.');
        }
        ...
        playerBoardWrappers.forEach((wrapper) => {
            ...
            for (let i = 1; i <= 5; i++) {
                ...
                lines.push(
                    lineElement ? Array.from(lineElement.querySelectorAll('.tile'))
                                    .map(tileEl => mapTileClassToColor(tileEl))
                                    .filter(color => color !== '') 
                                : []
                );
            ...
            // Wall (5x5)
            const wall: string[][] = Array(5).fill(null).map(() => Array(5).fill('')); // Initialize 5x5 empty wall
            const wallContainer = document.getElementById(`player-table-${bgaPlayerId}-wall`);
            if (wallContainer) {
                for (let r = 1; r <= 5; r++) {
                    for (let c = 1; c <= 5; c++) {
                        const wallSpot = document.getElementById(`player-table-${bgaPlayerId}-wall-spot-${r}-${c}`);
        ...
        const activeAvatarImg = document.querySelector('img.avatar.avatar_active[id^="avatar_active_"]');
        if (activeAvatarImg) {
        ...
        const opponentBoard = playerBoardsData.find((_, index) => index !== currentPlayerIndex) || playerBoardsData[1] || playerBoardsData[0];
...
const observer = new MutationObserver((_mutationsList, _obs) => {
    ...
    if (gameState) {
        chrome.runtime.sendMessage({
            ...
            if (error.message && !error.message.includes('Receiving end does not exist') && !error.message.includes('Could not establish connection. Receiving end does not exist.')) {
