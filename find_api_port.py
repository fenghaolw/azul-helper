...
def find_api_server_port(start_port: int = 5000, max_attempts: int = 10) -> Optional[dict]:
    """
...
def main():
    ...
    print(f"🔍 Searching for Azul AI API server starting from port {args.start_port}...")
    ...
    if server_info:
        if args.json:
            ...
            if health.get('agent_initialized'):
                print(f"   ✅ Agent initialized: {health.get('current_agent_type', 'unknown')}")
        ...
        if args.json:
            ...
            print(f"❌ No Azul AI API server found on ports {args.start_port}-{args.start_port + args.max_attempts - 1}")
