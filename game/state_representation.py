...
class AzulStateRepresentation:
    ...
    def _convert_from_game_state(self, game_state: GameState) -> None:
        ...
        self.pattern_lines = np.zeros(
            (StateConfig.MAX_PLAYERS, StateConfig.PATTERN_LINES, 8), dtype=np.float32
        )

        # Wall: shape (num_players, 5, 5) - binary filled/empty
        self.walls = np.zeros(
            (StateConfig.MAX_PLAYERS, StateConfig.WALL_SIZE, StateConfig.WALL_SIZE),
            dtype=np.int8,
        )

        # Floor lines: shape (num_players, 7, 7)
        # For each position: [has_tile, color_one_hot(5), first_player_marker]
        self.floor_lines = np.zeros(
            (StateConfig.MAX_PLAYERS, StateConfig.FLOOR_LINE_SIZE, 7), dtype=np.int8
        )
        ...
        self.factories = np.zeros(
            (StateConfig.MAX_FACTORIES, StateConfig.TILES_PER_FACTORY, 6), dtype=np.int8
        )
        ...
        self.center_first_player_marker = int(
            game_state.factory_area.center.has_first_player_marker
        )
    ...
    def _encode_player_board(self, player, player_idx: int) -> None:
        ...
        for line_idx, pattern_line in enumerate(player.pattern_lines):
            ...
            self.pattern_lines[player_idx, line_idx, 1] = len(pattern_line.tiles) / (
                line_idx + 1
            )
    ...
    def get_flat_state_vector(self, normalize: bool = True) -> np.ndarray:
        ...
        global_state = np.array(
            [
                ...
                (
                    self.round_number / StateConfig.MAX_ROUND
                    if normalize
                    else self.round_number
                ),
                self.game_over,  # Already 0/1
                (
                    (self.winner + 1) / self.num_players
                    if normalize and self.winner >= 0
                    else 0
                ),
            ],
            dtype=np.float32,
        ...
        components.append(
            np.array([self.center_first_player_marker], dtype=np.float32)
        )  # Already 0/1
    ...
    def get_state_dict(self) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            "center_first_player_marker": np.array(
                [self.center_first_player_marker], dtype=np.int8
            ),
            # Tile supply
    ...
    def get_player_view(
        self, player_id: int, include_hidden: bool = False
    ) -> Dict[str, np.ndarray]:
        """
        ...
        if not include_hidden:
            ...
            _ = np.sum(state["tile_supply"][1])  # discard_totals
            state["tile_supply"] = np.array(
                [[bag_totals, 0, 0, 0, 0], state["tile_supply"][1]], dtype=np.int32
            )
        ...
        if player_id != 0:
            ...
            state["current_player"] = np.array(
                [(self.current_player - player_id) % self.num_players], dtype=np.int32
            )
...
def get_state_documentation() -> str:
    ...
    - Note: All discarded tiles go to discard pile and are recycled back to bag
      when needed
