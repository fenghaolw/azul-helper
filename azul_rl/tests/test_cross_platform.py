...
class TestPlatformConsistency:
    ...
    def test_deterministic_game_creation(self):
        ...
        for seed in seeds:
            ...
            for i, game in enumerate(games[1:], 1):
                ...
                # Check factory setup
                assert len(game.factory_area.factories) == len(reference_game.factory_area.factories)
                
                for j, (factory, ref_factory) in enumerate(zip(game.factory_area.factories, reference_game.factory_area.factories)):
                    assert len(factory.tiles) == len(ref_factory.tiles)
                    for k, (tile, ref_tile) in enumerate(zip(factory.tiles, ref_factory.tiles)):
                        assert tile.color == ref_tile.color, \
                            f"Seed {seed}, game {i}, factory {j}, tile {k}: {tile.color} != {ref_tile.color}"
                ...
                assert bag_colors == ref_bag_colors, f"Bag contents differ for seed {seed}, game {i}"
    ...
    def test_state_representation_consistency(self):
        ...
        for i, state_repr in enumerate(state_reprs[1:], 1):
            ...
            np.testing.assert_array_equal(reference_vector, vector, 
    ...
    def test_numerical_precision_consistency(self):
        ...
        for player_idx in range(state_repr.num_players):
            for line_idx in range(5):
                ...
                precision_tests.append(('pattern_capacity', normalized_capacity, expected))
        ...
        print(f"✓ Numerical precision consistency verified ({len(precision_tests)} tests)")
    ...
    def test_hash_consistency(self):
        ...
        assert len(set(hashes)) == len(hashes), "Games with different seeds should have different hashes"
    ...
    def test_python_version_compatibility(self):
        ...
        if python_version >= (3, 8):
            ...
            if actions:
                ...
                assert (action := actions[0]) is not None
...
class TestDataTypeConsistency:
    ...
    def test_serialization_type_preservation(self):
        ...
        assert type(loaded_repr.current_player) == type(state_repr.current_player)
        ...
        original_vector = state_repr.get_flat_state_vector(normalize=True)
        np.testing.assert_array_almost_equal(original_vector, reconstructed_vector, decimal=6)
...
class TestRandomnessConsistency:
    ...
    def test_seed_reproducibility(self):
        ...
        for game in games[1:]:
            ...
            for ref_factory, factory in zip(reference_game.factory_area.factories, game.factory_area.factories):
                ref_colors = [tile.color for tile in ref_factory.tiles]
                colors = [tile.color for tile in factory.tiles]
                assert ref_colors == colors, "Factory contents should be identical with same seed"
            
            # Check bag contents
            ref_bag_colors = [tile.color for tile in reference_game.bag]
            bag_colors = [tile.color for tile in game.bag]
            assert ref_bag_colors == bag_colors, "Bag contents should be identical with same seed"
        ...
        for i, game1 in enumerate(games):
            for j, game2 in enumerate(games):
                if i != j:
                    ...
                    for factory1, factory2 in zip(game1.factory_area.factories, game2.factory_area.factories):
                    ...
                    assert factories_different, f"Games with seeds {seeds[i]} and {seeds[j]} should be different"
        
        print(f"✓ Different seeds produce different results verified ({len(seeds)} seeds)")
    ...
    def test_move_sequence_determinism(self):
        ...
        games = [GameState(num_players=2, seed=seed), GameState(num_players=2, seed=seed)]
        ...
        for move_num in range(10):
            ...
            if all(actions_list):
                ...
                assert len(actions1) == len(actions2), f"Different number of legal actions at move {move_num}"
                
                for action1, action2 in zip(actions1, actions2):
                    assert action1.source == action2.source, f"Action source differs at move {move_num}"
                    assert action1.color == action2.color, f"Action color differs at move {move_num}"
                    assert action1.destination == action2.destination, f"Action destination differs at move {move_num}"
