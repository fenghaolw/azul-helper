...
class TestRoundTransitions:
    ...
    def test_round_end_detection(self):
        ...
        assert not game.factory_area.is_round_over(), "Round should not be over initially"
        ...
        assert game.factory_area.is_round_over(), "Round should be over when all sources empty"
    ...
    def test_first_player_marker_transfer(self):
        ...
        assert game.current_player == 1, "Player with first player marker should go first"
        assert not game.players[1].has_first_player_marker(), "First player marker should be removed"
    ...
    def test_discard_pile_management(self):
        ...
        assert len(game.discard_pile) > initial_discard_size, "Discard pile should have excess tiles"
    ...
    def test_new_round_setup(self):
        ...
        assert game.factory_area.center.has_first_player_marker, "Center should have first player marker"
...
class TestGameTermination:
    ...
    def test_final_scoring_bonuses(self):
        ...
        assert game.players[0].score >= initial_score + expected_bonus, "Should apply final bonuses"
...
class TestTurnManagement:
    ...
    def test_turn_progression(self):
        ...
        for expected_player in [0, 1, 2, 0, 1]:  # Cycle through players
            assert game.current_player == expected_player, f"Should be player {expected_player}'s turn"
    ...
    def test_turn_does_not_change_on_round_end(self):
        ...
        # Current player should be determined by first player marker, not turn progression
        # (This test verifies the round end logic takes precedence)
        assert game.round_number > 1 or game.game_over, "Round should have ended or game over"
...
class TestComplexGameFlows:
    ...
    def test_complete_game_simulation(self):
        ...
        while not game.game_over and action_count < max_actions:
            ...
            # Prefer pattern line actions over floor line actions for better game progression
        ...
        assert action_count < max_actions, "Game should end within reasonable number of actions"
        
        # Check final state
        if game.game_over:
            assert game.winner is not None, "Winner should be determined"
            assert all(player.score >= 0 for player in game.players), "All scores should be non-negative"
    ...
    def test_bag_refill_from_discard(self):
        ...
        assert len(game.discard_pile) < initial_discard_size, "Discard pile should be reduced"
        ...
        total_factory_tiles = sum(len(factory.tiles) for factory in game.factory_area.factories)
        expected_tiles_used = game.factory_area.num_factories * 4
        assert total_factory_tiles == expected_tiles_used, "Factories should be properly filled"
    ...
    def test_multiple_round_progression(self):
        ...
        for round_idx in range(rounds_to_simulate):
            ...
            while not game.factory_area.is_round_over() and not game.game_over and action_count < max_actions_per_round:
                actions = game.get_legal_actions()
                if actions:
            ...
            assert action_count < max_actions_per_round, f"Round {round_idx + 1} took too many actions"
            
            if not game.game_over:
                game._end_round()
        
        expected_round = initial_round + rounds_to_simulate
        assert game.round_number == expected_round or game.game_over, \
    ...
    def test_edge_case_empty_bag_and_discard(self):
        ...
        for factory in game.factory_area.factories:
            assert len(factory.tiles) == 0, "Factories should be empty when no tiles available"
