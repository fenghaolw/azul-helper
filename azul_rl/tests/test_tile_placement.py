...
class TestPatternLinePlacement:
    ...
    def test_empty_pattern_line_accepts_any_color(self):
        ...
        for color in [TileColor.BLUE, TileColor.RED, TileColor.YELLOW, TileColor.BLACK, TileColor.WHITE]:
            tiles = [Tile(color)]
    ...
    def test_pattern_line_rejects_first_player_marker(self):
        ...
        assert not line.can_add_tiles(first_player_tile), "Pattern line should reject first player marker"
    ...
    def test_pattern_line_capacity_limits(self):
        ...
        assert not line.can_add_tiles(more_tiles), "Should reject tiles exceeding capacity"
    ...
    def test_pattern_line_overflow_handling(self):
        ...
        assert all(tile.color == TileColor.BLUE for tile in overflow), "Overflow should be same color"
    ...
    def test_pattern_line_clear_incomplete_line(self):
        ...
        assert len(discard_tiles) == 0, "Should not return discard tiles for incomplete line"
...
class TestWallPlacement:
    ...
    def test_wall_pattern_correctness(self):
        ...
        for row in wall.WALL_PATTERN:
            assert set(row) == set(colors), "Each row should contain all colors exactly once"
        
        for col in range(5):
            col_colors = [wall.WALL_PATTERN[row][col] for row in range(5)]
            assert set(col_colors) == set(colors), "Each column should contain all colors exactly once"
    ...
    def test_wall_tile_placement_validation(self):
        ...
        assert wall.can_place_tile(0, TileColor.BLUE), "Should be able to place blue in row 0"
        
        # Should be able to place yellow in row 0 (position 0,1)
        assert wall.can_place_tile(0, TileColor.YELLOW), "Should be able to place yellow in row 0"
        
        # Should not be able to place blue in row 1 (blue is at position 1,1)
        assert wall.can_place_tile(1, TileColor.BLUE), "Should be able to place blue in row 1"
    ...
    def test_wall_prevents_duplicate_placement(self):
        ...
        assert not wall.can_place_tile(0, TileColor.BLUE), "Should not place tile in filled position"
    ...
    def test_wall_invalid_row_handling(self):
        ...
        assert wall.place_tile(-1, TileColor.BLUE) == 0, "Invalid placement should return 0 points"
        assert wall.place_tile(5, TileColor.BLUE) == 0, "Invalid placement should return 0 points"
    ...
    def test_wall_color_completion_detection(self):
        ...
        assert not wall.is_color_complete(TileColor.RED), "Red color should not be complete"
...
class TestPlayerBoardPlacement:
    ...
    def test_valid_pattern_line_placement(self):
        ...
        assert len(board.pattern_lines[2].tiles) == 2, "Should place tiles on pattern line"
        assert board.pattern_lines[2].color == TileColor.BLUE, "Should set pattern line color"
    ...
    def test_pattern_line_overflow_to_floor(self):
        ...
        assert len(board.pattern_lines[1].tiles) == 0, "Pattern line should reject all tiles"
...
class TestComplexPlacementScenarios:
    ...
    def test_multiple_pattern_line_interactions(self):
        ...
        board.place_tiles_on_pattern_line(1, [Tile(TileColor.BLUE)] * 3)  # Wrong color + overflow
    ...
    def test_mixed_valid_invalid_placements(self):
        ...
        board.wall.filled[1][1] = True  # Blue position in row 1 filled (blue is at column 1 in row 1)
        
        # Try various placements
        board.place_tiles_on_pattern_line(0, [Tile(TileColor.RED)])    # Invalid: line full -> 1 tile to floor
        board.place_tiles_on_pattern_line(1, [Tile(TileColor.BLUE)])   # Invalid: wall filled -> 1 tile to floor  
        board.place_tiles_on_pattern_line(2, [Tile(TileColor.YELLOW)]) # Valid -> stays on pattern line
        board.place_tiles_on_pattern_line(3, [Tile(TileColor.BLACK)] * 5) # Invalid: too many tiles -> 5 tiles to floor
        ...
        assert board.pattern_lines[2].color == TileColor.YELLOW, "Valid placement should work"
        assert len(board.pattern_lines[3].tiles) == 0, "Invalid placement should be rejected"
