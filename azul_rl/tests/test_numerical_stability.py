...
class TestFloatingPointPrecision:
        ...
        for player_idx in range(state_repr.num_players):
            for line_idx in range(5):
                ...
                assert abs(normalized_capacity - expected_capacity) < 1e-10, \
                    f"Capacity normalization precision error: {normalized_capacity} != {expected_capacity}"
                
                # Test count normalization
                normalized_count = state_repr.pattern_lines[player_idx, line_idx, 1]
                assert 0.0 <= normalized_count <= 1.0, \
                ...
                if normalized_count > 0:
                    ...
                    assert abs(actual_count - round(actual_count)) < 1e-6, \
                        f"Count normalization precision error: {actual_count} not close to integer"
    ...
    def test_score_normalization_precision(self):
        ...
        for score in test_scores:
            ...
            assert abs(normalized_score - expected_normalized) < 1e-6, \
                f"Score normalization error for {score}: {normalized_score} != {expected_normalized}"
    ...
    def test_tile_count_precision(self):
        ...
        for location in ['bag', 'discard', 'factories', 'center', 'player_boards', 'walls']:
    ...
    def test_cumulative_precision_errors(self):
        ...
        initial_vector = initial_state_repr.get_flat_state_vector(normalize=True)
        ...
        for i in range(50):
            ...
            if actions:
                ...
                if i % 10 == 0:  # Sample every 10 moves
                    ...
                    assert np.all(vector >= -0.1), f"Values too negative at move {i}: min={vector.min()}"
                    assert np.all(vector <= 1.1), f"Values too large at move {i}: max={vector.max()}"
                    ...
                    for player_idx in range(state_repr.num_players):
                        for line_idx in range(5):
                            ...
                            assert abs(capacity - expected) < 1e-10, \
...
class TestNormalizationEdgeCases:
    ...
    def test_zero_division_safety(self):
        ...
        assert np.all(np.isfinite(empty_vector)), "Empty state contains non-finite values"
    ...
    def test_floating_point_edge_cases(self):
        ...
        for vector, name in [(norm_vector, "normalized"), (unnorm_vector, "unnormalized")]:
            # No NaN values
            ...
            if np.any(non_zero_mask):
                ...
                assert min_non_zero >= min_normal, \
...
class TestNumericalRobustness:
    ...
    def test_repeated_normalization_stability(self):
        ...
        np.testing.assert_array_equal(vector1, vector3,
        ...
        while not game.game_over and moves < max_moves:
            ...
            if actions:
                ...
                if moves % 20 == 0:
                    ...
                    assert np.all(np.isfinite(vector)), f"Non-finite values at move {moves}"
                    assert vector.dtype == np.float32, f"Wrong dtype at move {moves}: {vector.dtype}"
    ...
    def test_precision_across_different_seeds(self):
        ...
        assert all(v >= -0.1 for v in min_values), f"Inconsistent min values: {min_values}"
        assert all(v <= 1.1 for v in max_values), f"Inconsistent max values: {max_values}"
    ...
    def test_memory_layout_consistency(self):
        ...
        for key in dict1:
            np.testing.assert_array_equal(dict1[key], dict2[key], 
                                        err_msg=f"State dict mismatch for {key}")
...
class TestNumericalInvariants:
        ...
        for player_idx in range(state_repr.num_players):
            for line_idx in range(5):
                ...
                if empty_flag == 1:
                    ...
                    assert color_sum == 1.0, "Non-empty line should have exactly one color"
    ...
    def test_conservation_invariants(self):
        ...
        initial_state_repr = AzulStateRepresentation(game)
        initial_distribution = initial_state_repr.get_tile_distribution()
            ...
            if actions:
                ...
                for location, count in distribution.items():
                    if location not in ['total', 'expected_total']:
                        assert count >= 0, f"Negative tile count in {location} at move {move_num}: {count}"
    ...
    def test_monotonicity_invariants(self):
        ...
        for move_num in range(30):
            ...
            if actions:
                ...
                assert game.round_number >= previous_round, \
