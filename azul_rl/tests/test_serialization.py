...
class TestStateSerialization:
    ...
    def test_game_state_pickle_serialization(self):
        ...
        try:
            ...
            for i, (orig_player, loaded_player) in enumerate(zip(original_game.players, loaded_game.players)):
                assert orig_player.score == loaded_player.score
                assert len(orig_player.floor_line) == len(loaded_player.floor_line)
                
                # Check pattern lines
                for j, (orig_line, loaded_line) in enumerate(zip(orig_player.pattern_lines, loaded_player.pattern_lines)):
                ...
                for row in range(5):
                    for col in range(5):
                        assert orig_player.wall.filled[row][col] == loaded_player.wall.filled[row][col]
            
            # Verify factories
            for orig_factory, loaded_factory in zip(original_game.factory_area.factories, loaded_game.factory_area.factories):
                assert len(orig_factory.tiles) == len(loaded_factory.tiles)
                for orig_tile, loaded_tile in zip(orig_factory.tiles, loaded_factory.tiles):
                    assert orig_tile.color == loaded_tile.color
            
            # Verify center
            assert len(original_game.factory_area.center.tiles) == len(loaded_game.factory_area.center.tiles)
            assert original_game.factory_area.center.has_first_player_marker == loaded_game.factory_area.center.has_first_player_marker
    ...
    def test_partial_state_reconstruction(self):
        ...
        try:
            ...
            reconstructed_info = {
                'player_scores': loaded_state_repr.player_scores[:loaded_state_repr.num_players].tolist(),
                ...
                'center_has_first_player': bool(loaded_state_repr.center_first_player_marker),
            }
            
            # Verify reconstruction accuracy
            for key in original_info:
                assert original_info[key] == reconstructed_info[key], \
...
class TestSerializationEdgeCases:
    ...
    def test_completed_game_serialization(self):
        ...
        if game.game_over:
            ...
            print("⚠ Game didn't complete within move limit, skipping completed game test")
    ...
    def test_large_state_serialization(self):
        ...
        try:
            ...
            print(f"✓ Large state serialization test passed (serialize: {serialize_time:.3f}s, load: {load_time:.3f}s)")
