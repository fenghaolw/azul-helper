...
class TestBasicPerformance:
    ...
    def test_game_creation_performance(self):
        ...
        for num_players in player_counts:
            ...
            for i in range(num_iterations):
                game = GameState(num_players=num_players, seed=42 + i)
            ...
            assert avg_time < 0.001, f"Game creation too slow for {num_players} players: {avg_time:.6f}s"
    ...
    def test_state_representation_creation_performance(self):
        ...
        for scenario_name, num_moves in test_scenarios:
            ...
            assert avg_time < 0.005, f"State representation creation too slow for {scenario_name}: {avg_time:.6f}s"
    ...
    def test_flat_vector_generation_performance(self):
        ...
        for vector_type, normalize in vector_types:
            ...
            for _ in range(num_iterations):
                vector = state_repr.get_flat_state_vector(normalize=normalize)
            ...
            assert avg_time < 0.001, f"Vector generation too slow for {vector_type}: {avg_time:.6f}s"
    ...
    def test_legal_actions_performance(self):
        ...
        for move_num in range(50):
            ...
            assert avg_time < 0.002, f"Legal actions too slow at move {move_num}: {avg_time:.6f}s"
        ...
        print(f"✓ Legal actions performance: {avg_action_time:.6f}s average ({len(action_times)} samples)")
    ...
    def test_action_application_performance(self):
        ...
        for move_num in range(30):
            ...
            assert avg_time < 0.005, f"Action application too slow at move {move_num}: {avg_time:.6f}s"
            
            # Apply action to main game
            game.apply_action(action)
        
        avg_application_time = np.mean(application_times)
        print(f"✓ Action application performance: {avg_application_time:.6f}s average ({len(application_times)} samples)")
...
class TestScalabilityPerformance:
    ...
    def test_player_count_scaling(self):
        ...
        assert scaling_factor < 3.0, f"Poor scaling: 4p is {scaling_factor:.2f}x slower than 2p"
        
        print(f"✓ Player count scaling: {scaling_results}, factor: {scaling_factor:.2f}")
    ...
    def test_game_length_scaling(self):
        ...
        assert degradation_factor < 2.0, f"Performance degrades too much: {degradation_factor:.2f}x"
        
        print(f"✓ Game length scaling: {length_results}, degradation: {degradation_factor:.2f}x")
    ...
    def test_batch_processing_performance(self):
        ...
        assert overhead_factor < 1.5, f"Batch processing overhead too high: {overhead_factor:.2f}x"
        
        print(f"✓ Batch processing performance: {batch_results}, overhead: {overhead_factor:.2f}x")
...
class TestMemoryPerformance:
    ...
    def test_memory_usage_scaling(self):
        ...
        assert avg_memory_per_game < 1.0, f"Memory usage too high: {avg_memory_per_game:.3f} MB per game"
        
        print(f"✓ Memory usage scaling: {memory_results}, average: {avg_memory_per_game:.3f} MB/game")
    ...
    def test_memory_leak_detection(self):
        ...
        for iteration in range(100):
            ...
            if iteration % 20 == 19:
                ...
                assert memory_growth < 10.0, f"Potential memory leak: {memory_growth:.3f} MB growth at iteration {iteration}"
    ...
    def test_large_state_memory_efficiency(self):
        ...
        assert memory_per_state < 0.1, f"State representation too large: {memory_per_state:.3f} MB"
...
class TestConcurrentPerformance:
    ...
    def test_independent_game_performance(self):
        ...
        assert time_per_game < 0.05, f"Independent games too slow: {time_per_game:.6f}s per game"
    ...
    def test_state_representation_independence(self):
        ...
        assert time_per_repr < 0.01, f"Concurrent state representations too slow: {time_per_repr:.6f}s"
        ...
        print(f"✓ State representation independence: {time_per_repr:.6f}s per representation")
...
class TestPerformanceRegression:
    ...
    def test_baseline_performance_benchmarks(self):
        ...
        for operation, time_taken in benchmarks.items():
            ...
            assert time_taken < limit, f"{operation} too slow: {time_taken:.6f}s > {limit}s"
