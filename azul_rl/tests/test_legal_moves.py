...
from game.factory import Factory, CenterArea
...
class TestPatternLineConstraints:
    ...
    def test_cannot_place_wrong_color_on_pattern_line(self):
        ...
        line_1_non_blue = [a for a in actions 
                          if a.destination == 1 and a.color != TileColor.BLUE]
    ...
    def test_cannot_place_on_wall_filled_position(self):
        ...
        blue_line_0_actions = [a for a in actions 
                              if a.destination == 0 and a.color == TileColor.BLUE]
        assert len(blue_line_0_actions) == 0, "Should not place tiles for filled wall position"
    ...
    def test_pattern_line_capacity_constraints(self):
        ...
        # - Floor line
        blue_factory_0_actions = [a for a in actions 
                                 if a.source == 0 and a.color == TileColor.BLUE]
...
class TestComplexScenarios:
    ...
    def test_round_ending_scenario(self):
        ...
        last_factory_actions = [a for a in actions if a.source == len(game.factory_area.factories) - 1]
    ...
    def test_all_pattern_lines_blocked(self):
        ...
        assert destinations == {-1}, "Should only have floor line moves when all lines blocked"
...
class TestEdgeCases:
    ...
    def test_first_player_marker_only_in_center(self):
        ...
        assert len(center_actions) == 0, "Should not have moves for first player marker only"
    ...
    def test_player_specific_legal_moves(self):
        ...
        p0_blue_line_0 = any(a.destination == 0 and a.color == TileColor.BLUE 
                            for a in player_0_actions)
        p1_blue_line_0 = any(a.destination == 0 and a.color == TileColor.BLUE 
                            for a in player_1_actions)
        
        # This test depends on the specific factory setup, so we just check they're different
        assert player_0_actions != player_1_actions or len(player_0_actions) == 0, \
