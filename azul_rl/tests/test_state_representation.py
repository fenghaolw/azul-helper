...
class TestStateRepresentation:
    ...
    def test_state_shapes(self):
        ...
        expected_shapes = {
            ...
            "walls": (
                StateConfig.MAX_PLAYERS,
                StateConfig.WALL_SIZE,
                StateConfig.WALL_SIZE,
            ),
    ...
    def test_flat_state_vector(self):
        ...
        assert np.all(
            flat_vector >= -0.1
        )  # Allow small negative values for some components
    ...
    def test_state_dict(self):
        ...
        expected_keys = {
            ...
            "walls",
            "floor_lines",
            "first_player_markers",
            "factories",
            "center_tiles",
            "center_first_player_marker",
            "tile_supply",
        }
    ...
    def test_color_encoding(self):
        ...
        assert (
            state_repr._color_to_index(TileColor.FIRST_PLAYER)
            == ColorIndex.FIRST_PLAYER
        )
    ...
    def test_tile_supply_encoding(self):
        ...
        assert (
            distribution["factories"] == state_repr.num_factories * 4
        )  # Each factory has 4 tiles initially
        ...
        assert (
            new_distribution["total"] == new_distribution["expected_total"]
    ...
    def test_state_size_calculation(self):
        ...
        expected_size = (
            ...
            + StateConfig.MAX_PLAYERS
            * StateConfig.WALL_SIZE
            * StateConfig.WALL_SIZE  # Walls
            + StateConfig.MAX_PLAYERS
            * StateConfig.FLOOR_LINE_SIZE
            * 7  # Floor lines (updated)
    ...
    def test_color_representation_accuracy(self):
            ...
            if factory_idx < state_repr.num_factories:
                ...
                for tile_idx, tile in enumerate(factory.tiles):
                    if tile_idx < 4:  # Max 4 tiles per factory
                        ...
                        assert (
                            has_tile_encoded == 1
                        ), f"Factory {factory_idx}, tile {tile_idx} should be marked as present"
        ...
        for tile_idx, tile in enumerate(center_tiles):
            if (
                tile_idx < len(state_repr.center_tiles)
                and tile.color != TileColor.FIRST_PLAYER
            ):
                ...
                assert (
                    encoded_tile[0] == 1
                ), f"Center tile {tile_idx} should be marked as present"
            ...
            if player_idx < state_repr.num_players:
                ...
                for line_idx, pattern_line in enumerate(player.pattern_lines):
                    ...
                    if pattern_line.color is not None:
                        ...
                        expected_color_idx = state_repr._color_to_index(
                            pattern_line.color
                        )
                        if expected_color_idx < 5:
                            color_encoding = encoded_line[
                                2:7
                            ]  # Colors are at indices 2-6
                            assert (
                                color_encoding[expected_color_idx] == 1
                            ...
                            expected_count = len(pattern_line.tiles) / (
                                line_idx + 1
                            )  # Normalized
                            actual_count = encoded_line[1]
                            assert (
                                abs(expected_count - actual_count) < 0.01
                            ), f"Player {player_idx}, pattern line {line_idx}: tile count mismatch"

                            print(
                                f"  ✓ Player {player_idx}, line {line_idx}: {pattern_line.color.value} with {len(pattern_line.tiles)} tiles"
                            )
                    else:
                ...
                for pos, tile in enumerate(player.floor_line):
                    if pos < len(state_repr.floor_lines[player_idx]):
                        ...
                        assert (
                            encoded_pos[0] == 1
                        ), f"Player {player_idx}, floor position {pos} should be marked as occupied"
        ...
        assert (
            pattern_lines_with_tiles > 0
        ), "Test should have at least some tiles in pattern lines for comprehensive validation"
    ...
    def test_wall_pattern_accuracy(self):
        ...
        for player_idx, player in enumerate(game.players):
            if player_idx < state_repr.num_players:
                ...
                print(
                    f"    Wall tiles: actual={actual_wall_tiles}, encoded={encoded_wall_tiles}"
                )

                assert (
                    actual_wall_tiles == encoded_wall_tiles
...
if __name__ == "__main__":
    ...
    state_repr = AzulStateRepresentation(game)
    print("State representation created")
