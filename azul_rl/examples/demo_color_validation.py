...
from azul_rl.game import AzulStateRepresentation, GameState, TileColor
                ...
                for line_idx in range(5):
                    ...
                    if line_array[6] == 0:  # Not empty
                        ...
                        if color_encoding[:5].sum() > 0:
                            ...
                            tile_count = round(
                                line_array[1] * (line_idx + 1)
                            )  # Denormalize count
                            pattern_colors.append(
                                f"line_{line_idx}:{color_names[color_idx]}"
                                f"({tile_count})"
                            )
    ...
    for player_idx, player in enumerate(game.players):
        if player_idx < final_state_repr.num_players:
            ...
            for line_idx, pattern_line in enumerate(player.pattern_lines):
                actual_color = (
                    pattern_line.color.value if pattern_line.color else "empty"
                )
                ...
                if actual_color != encoded_color:
                    print(
                        f"  Line {line_idx}: actual={actual_color}({tile_count}), "
                        f"encoded={encoded_color} {match}"
                    )
                    print(f"    Debug: encoded_line={encoded_line}")
                    print(
                        f"    Debug: color_encoding={color_encoding[:5]}, "
                        f"empty_flag={encoded_line[6]}"
                    )
                else:
                    print(
                        f"  Line {line_idx}: actual={actual_color}({tile_count}), "
