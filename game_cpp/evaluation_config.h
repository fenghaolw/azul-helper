...
struct EvaluationConfig {
    ...
    bool deterministic_evaluation = true; // Use deterministic agent actions if possible
    bool swap_player_positions = true; // Evaluate with agents in different starting positions
...
struct GameResult {
    ...
    GameResult(int id, int win, const std::vector<int>& scores, int rounds, int moves, double duration)
        : game_id(id), winner(win), final_scores(scores), num_rounds(rounds), 
          total_moves(moves), game_duration(duration) {}
};
...
struct EvaluationResult {
    ...
    // Constructor
    EvaluationResult(const std::string& test_name, const std::string& baseline_name, const EvaluationConfig& cfg)
        : test_agent_name(test_name), baseline_agent_name(baseline_name), config(cfg),
          games_played(0), test_agent_wins(0), baseline_agent_wins(0), draws(0),
          test_agent_win_rate(0.0), baseline_agent_win_rate(0.0),
          test_agent_avg_score(0.0), baseline_agent_avg_score(0.0),
          average_score_difference(0.0), average_game_duration(0.0),
          confidence_interval(0.0, 0.0), p_value(1.0), is_statistically_significant(false) {
