...
class AgentEvaluator {
 public:
   ...
   */
  EvaluationResult evaluate_agent(EvaluationAgent& test_agent,
                                  EvaluationAgent& baseline_agent);

  /**
   * Quick evaluation with fewer games for rapid testing.
   */
  EvaluationResult quick_evaluation(EvaluationAgent& test_agent,
                                    EvaluationAgent& baseline_agent,
                                    int num_games = 20);
...
std::unique_ptr<EvaluationAgent> create_minimax_evaluation_agent(
    int depth = 4, const std::string& name = "MinimaxAgent");

std::unique_ptr<EvaluationAgent> create_mcts_evaluation_agent(
    int num_simulations = 1000, double uct_c = 1.4, int seed = -1,
