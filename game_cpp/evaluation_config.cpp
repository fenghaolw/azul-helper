...
void EvaluationResult::finalize_results() {
    if (games_played > 0) {
        ...
        baseline_agent_win_rate = static_cast<double>(baseline_agent_wins) / games_played;
        ...
        for (const auto& game_result : game_results) {
            if (game_result.final_scores.size() >= 2) {
                ...
                total_score_diff += (game_result.final_scores[0] - game_result.final_scores[1]);
            }
...
std::string EvaluationResult::summary() const {
    ...
    oss << "Evaluation Results: " << test_agent_name << " vs " << baseline_agent_name << "\n";
    ...
    oss << "  " << baseline_agent_name << ": " << std::fixed << std::setprecision(1) 
        << (baseline_agent_win_rate * 100) << "% (" << baseline_agent_wins << " wins)\n";
    oss << "  Draws: " << draws << "\n\n";
    
    oss << "Performance Metrics:\n";
    oss << "  Average Score Difference: " << std::showpos << std::fixed << std::setprecision(1) 
    ...
    oss << "  Statistically Significant: " << (is_statistically_significant ? "Yes" : "No") << "\n";
...
void TournamentResult::calculate_rankings() {
    ...
    // Sort by win rate (descending), then by average score difference (descending)
