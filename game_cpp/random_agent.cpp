...
std::vector<double> RandomAgent::get_action_probabilities(
...
int main() {
  ...
  try {
    ...
    std::cout << "🎮 Game: " << game_type.short_name << " ("
              << (game_type.utility == open_spiel::GameType::Utility::kZeroSum
                      ? "Zero-Sum"
                      : "General-Sum")
              << ")" << std::endl;
    ...
    std::cout << "🤖 Created RandomAgent (Player 0) and MinimaxAgent (Player "
                 "1, depth=3)"
              << std::endl;
    ...
    while (!state->IsTerminal() && turn_count < max_turns) {
      ...
      if (current_player == 0) {
        ...
        try {
          ...
          std::cout << "📊 MinimaxAgent explored "
                    << minimax_agent.nodes_explored() << " nodes" << std::endl;
        } catch (const std::exception& e) {
          std::cerr << "⚠️  MinimaxAgent failed (" << e.what()
                    << "), using random fallback" << std::endl;
          auto legal_actions = state->LegalActions();
          if (!legal_actions.empty()) {
      ...
      // Apply the action
      std::cout << "🎯 " << agent_name << " chose action: " << chosen_action
                << std::endl;
    ...
    if (state->IsTerminal()) {
      ...
      std::cout << "⏰ Game stopped after " << max_turns
                << " turns (demonstration limit)" << std::endl;
    }

    std::cout << "\n✅ Integration test completed successfully!" << std::endl;
    std::cout << "💡 Key observations:" << std::endl;
    std::cout << "   • Both agents can play against OpenSpiel Azul game"
              << std::endl;
    std::cout << "   • MinimaxAgent "
              << (game_type.utility == open_spiel::GameType::Utility::kZeroSum
                      ? "benefits from proper zero-sum algorithms"
                      : "works despite general-sum classification")
              << std::endl;
    std::cout << "   • Random vs Minimax provides good performance baseline"
              << std::endl;
